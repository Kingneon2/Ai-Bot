...
def is_member(user_id: int) -> bool:
    ...
    if not res.get("ok"):
        # Safe fallback: if bot is not admin or channel check fails, allow access
        return True
...
def handle_callback(cq):
    ...
    if data == "menu:close":
        ...
        u["mode"] = "image"
        tg("sendMessage", chat_id=chat_id, text="Image mode on. Send a prompt (e.g. 'A futuristic city') and I'll generate it.")
    elif data == "menu:video":
        u["mode"] = "video"
        tg("sendMessage", chat_id=chat_id, text="Video mode on. Send a prompt (e.g. 'A running dog') and I'll generate a video clip.")
...
def handle_text(chat_id, user_id, text):
    # Respond to /start or /menu commands immediately
